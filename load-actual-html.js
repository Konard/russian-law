...
const url = `http://actual.pravo.gov.ru:8000/api/ebpi/redtext/?t=${options.sourceDocumentId}`;
...
axios({
  ...
  .then(function (response) {
    const json = JSON.parse(iconv.decode(Buffer.from(response.data), 'utf-8'));
    const html = transformHtml(json.RedText);
